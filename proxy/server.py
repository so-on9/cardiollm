...
def auth_ok(request: Request, x_api_key: str | None) -> bool:
    if x_api_key and hmac.compare_digest(x_api_key, API_KEY): return True
    token = request.cookies.get(COOKIE_NAME)
    if token and parse_token(token): return True
    return False
...
def untag_translated_lines(text: str) -> str:
    ...
    if "[L" in text:
        ...
        for seg in segments:
            ...
            if seg: out_lines.append(seg)
        if out_lines: return "\n".join(out_lines)
    s = text.strip()
    if not s: return ""
    s = " ".join(s.split())
    sentences = [p.strip() for p in re.split(r'(?<=[。！？；])', s) if p.strip()]
    keywords = ("左心", "右心", "心房", "心室", "主動脈", "肺動脈", "二尖瓣", "三尖瓣", "主動脈瓣", "肺動脈瓣", "心包膜", "心肌", "心功能", "收縮功能", "舒張功能")
    lines: list[str] = []
...
TERM_MAP = {"連枷樣運動": "飄動樣運動", "連枷樣": "飄動樣", "連枷運動": "飄動運動", "連枷": "飄動"}
def apply_term_map(text: str) -> str:
    if not text: return text
    for src, tgt in TERM_MAP.items(): text = text.replace(src, tgt)
    return text

class GlossaryPair(BaseModel): src: str; tgt: str
...
def build_translate_prompt(req: TranslateReq) -> str:
    ...
    if "clinical" in style: tone_prompt = "使用臨床報告語氣，專業且精簡。"
    elif "academic" in style: tone_prompt = "使用學術性中文，保持邏輯嚴謹與精確。"
    elif "patient" in style or "friendly" in style: tone_prompt = "使用淺顯易懂的病患友善中文，保持醫學準確。"
    else: tone_prompt = "使用標準專業繁體中文，保持報告格式清晰。"
...
def build_summary_prompt(req: SummarizeReq) -> str:
    style = (req.style or "").strip().lower()
    if "clinical" in style: tone_prompt = "Write a concise structured clinical summary focusing on cardiac function, abnormalities, and key impressions."
    elif "academic" in style: tone_prompt = "Summarize the report in an academic tone, highlighting methodology, results, and interpretations."
    elif "patient" in style or "friendly" in style: tone_prompt = "Explain the findings in simple, patient-friendly Traditional Chinese that non-medical readers can understand."
    else: tone_prompt = "Provide a brief, professional summary of the cardiac ultrasound findings."
    return (
...
def ollama_generate(model: str, prompt: str, max_new_tokens: int, temperature: float, top_p: float) -> str:
    url = f"{OLLAMA_URL}/api/generate"
    payload = {
        "model": model, "prompt": prompt, "stream": False, "keep_alive": KEEP_ALIVE,
        "options": {"temperature": float(temperature), "top_p": float(top_p), "num_predict": int(max_new_tokens), "repeat_penalty": 1.05, "num_ctx": 4096}
    }
...
def build_mistral_translate_prompt(english_text: str, tone: str = "Clinical") -> str:
    ...
    if tone == "clinical": tone_hint = "Use formal clinical tone.\n"
    elif tone == "academic": tone_hint = "Use academic, publication-style language suitable for medical journals.\n"
    elif tone in ["patient-friendly", "simple"]: tone_hint = "Use simple, patient-friendly tone understandable by non-medical readers.\n"
    else: tone_hint = ""
...
def build_mistral_summary_prompt(chinese_text: str, style: str = "Clinical") -> str:
    ...
    if style == "clinical": style_hint = "- Use concise and professional clinical language.\n- Follow hospital reporting style.\n"
    elif style == "academic": style_hint = "- Use formal academic tone suitable for scientific discussion.\n- Explain findings with logical reasoning, including possible physiological implications.\n"
    elif style in ["patient-friendly", "simple"]: style_hint = "- Explain findings in plain language for patient understanding.\n- Simplify medical terms while keeping accuracy.\n"
    else: style_hint = ""
...
def healthz():
    ok = False
    try: ok = requests.get(f"{OLLAMA_URL}/api/tags", timeout=3).status_code == 200
    except Exception: ok = False
    return {"ok": ok, "translator_default": MODEL_TRANS_DEFAULT, "summarizer_default": MODEL_SUM_DEFAULT}

@app.get("/models")
def models(request: Request, x_api_key: str = Header(default=None)):
    if not auth_ok(request, x_api_key): raise HTTPException(401, "unauthorized")
    r = requests.get(f"{OLLAMA_URL}/api/tags", timeout=10)
    data = r.json()
    names = sorted({m.get("name","") for m in data.get("models", []) if m.get("name")})
    return {"names": names, "defaults": {"translator": MODEL_TRANS_DEFAULT, "summarizer": MODEL_SUM_DEFAULT}}

@app.post("/translate")
def translate(req: TranslateReq, request: Request, x_api_key: str = Header(default=None)):
    if not auth_ok(request, x_api_key): raise HTTPException(401, "unauthorized")
    model = req.translator_model or MODEL_TRANS_DEFAULT
    if "mistralv0.1" in model: prompt = build_mistral_v01_translate_prompt(req)
    else: prompt = build_translate_prompt(req)
    raw_text = ollama_generate(model, prompt, req.max_new_tokens, req.temperature, req.top_p)
    text = untag_translated_lines(raw_text)
    text = apply_term_map(text)
    text = strip_model_header(text)
    src_set = extract_nums_units(req.source); tgt_set = extract_nums_units(text)
    missing = sorted(src_set - tgt_set); extra = sorted(tgt_set - src_set)
    warn = ""
    if missing: warn += "譯文檢查：缺 " + ", ".join(missing)
    if extra: warn += ("\n" if warn else "") + "譯文檢查：多 " + ", ".join(extra)
    return {"translation": text, "warn": warn}

@app.post("/translate_stream")
def translate_stream(req: TranslateReq, request: Request, x_api_key: str = Header(default=None)):
    if not auth_ok(request, x_api_key): raise HTTPException(401, "unauthorized")
    model = req.translator_model or MODEL_TRANS_DEFAULT
    if "mistralv0.1" in model: prompt = build_mistral_v01_translate_prompt(req)
    else: prompt = build_translate_prompt(req)
    ...
    payload = {
        ...
        "options": {"temperature": float(req.temperature), "top_p": float(req.top_p), "num_predict": int(req.max_new_tokens), "repeat_penalty": 1.05, "num_ctx": 4096},
    }
    def event_gen():
        try: r = requests.post(url, json=payload, stream=True, timeout=600)
        ...
        for line in r.iter_lines():
            ...
            if token:
                full_text += token
                yield json.dumps({"event": "token", "delta": token}, ensure_ascii=False) + "\n"
            if done:
                src_set = extract_nums_units(req.source); tgt_set = extract_nums_units(full_text)
                missing = sorted(src_set - tgt_set); extra = sorted(tgt_set - src_set)
                warn = ""
                if missing: warn += "譯文檢查：缺 " + ", ".join(missing)
                if extra: warn += ("\n" if warn else "") + "譯文檢查：多 " + ", ".join(extra)
                yield json.dumps({"event": "done", "warn": warn}, ensure_ascii=False) + "\n"
                break
    return StreamingResponse(event_gen(), media_type="text/plain; charset=utf-8")

@app.post("/summarize")
def summarize(req: SummarizeReq, request: Request, x_api_key: str = Header(default=None)):
    if not auth_ok(request, x_api_key): raise HTTPException(401, "unauthorized")
    model = req.summarizer_model or MODEL_SUM_DEFAULT
    if "mistralv0.1" in model: prompt = build_mistral_v01_summary_prompt(req)
    else: prompt = build_summary_prompt(req)
    text = ollama_generate(model, prompt, req.max_new_tokens, req.temperature, req.top_p).strip()
    text = dedup_summary_lines(text)
    src_set = extract_nums_units(req.source); tgt_set = extract_nums_units(text)
    missing = sorted(src_set - tgt_set); extra = sorted(tgt_set - src_set)
    warn = ""
    if missing: warn += "摘要檢查：缺 " + ", ".join(missing)
...
def pipeline(req: PipelineReq, request: Request, x_api_key: str = Header(default=None)):
    ...
    if mistral_trans: t_prompt = build_mistral_translate_prompt(req.source, req.style)
    else:
        t_prompt = build_translate_prompt(TranslateReq(
            source=req.source, target_lang=req.target_lang, style=req.style,
            keep_formatting=req.keep_formatting, glossary=req.glossary,
            max_new_tokens=req.max_new_tokens_translate, temperature=req.temperature_translate, top_p=req.top_p,
        ))
    raw_t = ollama_generate(t_model, t_prompt, req.max_new_tokens_translate, req.temperature_translate, req.top_p)
    t = untag_translated_lines(raw_t)
    t = apply_term_map(t)
    if mistral_sum: s_prompt = build_mistral_summary_prompt(t, req.style)
    else:
        s_prompt = build_summary_prompt(SummarizeReq(
            source=t, target_lang=req.target_lang, style=req.style,
            keep_formatting=req.keep_formatting, max_new_tokens=req.max_new_tokens_summary,
            temperature=req.temperature_summary, top_p=req.top_p,
        ))
    s = ollama_generate(s_model, s_prompt, req.max_new_tokens_summary, req.temperature_summary, req.top_p).strip()
    src_set = extract_nums_units(req.source); trans_set = extract_nums_units(t); sum_set = extract_nums_units(s)
    return {
        "translation": t, "summary": s,
        "warn_translation_missing": sorted(src_set - trans_set), "warn_translation_extra": sorted(trans_set - src_set),
        "warn_summary_missing": sorted(trans_set - sum_set), "warn_summary_extra": sorted(sum_set - trans_set),
    }
